...
# avi ctrl creation
#
list_vm=$(govc find -json -type m -name "${avi_ctrl_name_2}")
if [[ ${list_vm} != "null" ]] ; then
  echo "ERROR: unable to create VM ${avi_ctrl_name_2}: it already exists"
else
  #
  # Avi options
  #
  ...
  avi_options=$(echo ${avi_options} | jq '.PropertyMapping[0] += {"Value": "'${ip_avi_2}'"}')
  ...
  avi_options=$(echo ${avi_options} | jq '.PropertyMapping[11] += {"Value": "'${avi_ctrl_name_2}'"}')
  avi_options=$(echo ${avi_options} | jq '.NetworkMapping[0] += {"Network": "'${network_avi}'"}')
  avi_options=$(echo ${avi_options} | jq '. += {"Name": "'${avi_ctrl_name_2}'"}')
  echo ${avi_options} | jq -c -r '.' | tee /home/ubuntu/json/options-${avi_ctrl_name_2}.json
  #
  # Avi Creation
  #
  govc import.ova --options="/home/ubuntu/json/options-${avi_ctrl_name_2}.json" -folder "${folder_avi}" "/home/ubuntu/bin/$(basename ${avi_ova_url})" > /dev/null
  govc vm.power -on=true "${avi_ctrl_name_2}" > /dev/null
