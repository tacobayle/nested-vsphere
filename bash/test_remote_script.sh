test_remote_script() {
  ...
  local ip_gw="${1}"
  local script_file="${2}"
  local jsonFile_remote="${3}"
  ssh -o StrictHostKeyChecking=no "ubuntu@${ip_gw}" "test -f ${script_file%.*}.done" 2>/dev/null
  if [[ $? -eq 0 ]]; then
    echo "${script_file} has already been executed until the end"
    exit
  fi
  ssh -o StrictHostKeyChecking=no -t ubuntu@${ip_gw} "${script_file} ${jsonFile_remote} ${script_file%.*}.done" 2>&1 &
  while true ; do
