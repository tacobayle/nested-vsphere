...
log_message "${deployment_name}: Deployment of NSX manager  - This should take about 20 minutes" "" "${slack_webhook}" "${google_webhook}"
#
# NSX download
#
...
govc about
if [ $? -ne 0 ] ; then
  log_message "ERROR: unable to connect to vCenter" "" "" ""
  exit
fi
#
# folder creation
...
log_message "${deployment_name}: Creation of a folder for the NSX Manager" "" "" ""
if $(echo ${list_folder} | jq -e '. | any(. == "./vm/'${folder_nsx}'")' >/dev/null ) ; then
  log_message "${deployment_name}: ERROR: unable to create folder ${folder_nsx}: it already exists" "" "" ""
else
...
if [[ ${list_vm} != "null" ]] ; then
  ...
  log_message "${deployment_name}: NSX Manager deployed" "" "${slack_webhook}" "${google_webhook}"
  log_message "${deployment_name}:  +++ pausing for 180 seconds" "" "" ""
  sleep 180
  #
  # NSX HTTPS check
  #
  ...
  do
    log_message "${deployment_name}: Attempt $count: Waiting for NSX Manager to be reachable..." "" "" ""
    sleep 30
    count=$((count+1))
    if [ ${count} = 60 ]; then
      log_message "${deployment_name}: ERROR: Unable to connect to NSX Manager" "" "${slack_webhook}" "${google_webhook}"
      exit
    fi
  done
